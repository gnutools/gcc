...
#define ARRAY_ELEM(type, array, idx) \
    *((type *) ((char *) array->base_addr + idx * GFC_DESCRIPTOR_SPACING (array, 0)))

#define ARRAY_ELEM_I4(array, idx) ARRAY_ELEM(GFC_INTEGER_4, array, idx)
#define ARRAY_ELEM_I8(array, idx) ARRAY_ELEM(GFC_INTEGER_8, array, idx)
  ...
  if (val == 0)
    {
      ...
      for (int i = 0; i < 13; i++)
	ARRAY_ELEM_I4(values, i) = -1;

      /* Device ID  */
      if (sb.st_dev <= INT32_MAX)
	ARRAY_ELEM_I4(values, 0) = sb.st_dev;

      /* Inode number  */
      if (sb.st_ino <= INT32_MAX)
	ARRAY_ELEM_I4(values, 1) = sb.st_ino;

      /* File mode  */
      if (sb.st_mode <= INT32_MAX)
	ARRAY_ELEM_I4(values, 2) = sb.st_mode;

      /* Number of (hard) links  */
      if (sb.st_nlink <= INT32_MAX)
	ARRAY_ELEM_I4(values, 3) = sb.st_nlink;

      /* Owner's uid  */
      if (sb.st_uid <= INT32_MAX)
	ARRAY_ELEM_I4(values, 4) = sb.st_uid;

      /* Owner's gid  */
      if (sb.st_gid <= INT32_MAX)
	ARRAY_ELEM_I4(values, 5) = sb.st_gid;

      /* ID of device containing directory entry for file (0 if not available) */
#if HAVE_STRUCT_STAT_ST_RDEV
      if (sb.st_rdev <= INT32_MAX)
	ARRAY_ELEM_I4(values, 6) = sb.st_rdev;
#else
      ARRAY_ELEM_I4(values, 6) = 0;
#endif

      /* File size (bytes)  */
      if (sb.st_size <= INT32_MAX)
	ARRAY_ELEM_I4(values, 7) = sb.st_size;

      /* Last access time  */
      if (sb.st_atime <= INT32_MAX)
	ARRAY_ELEM_I4(values, 8) = sb.st_atime;

      /* Last modification time  */
      if (sb.st_mtime <= INT32_MAX)
	ARRAY_ELEM_I4(values, 9) = sb.st_mtime;

      /* Last file status change time  */
      if (sb.st_ctime <= INT32_MAX)
	ARRAY_ELEM_I4(values, 10) = sb.st_ctime;

      /* Preferred I/O block size (-1 if not available)  */
#if HAVE_STRUCT_STAT_ST_BLKSIZE
      if (sb.st_blksize <= INT32_MAX)
	ARRAY_ELEM_I4(values, 11) = sb.st_blksize;
#endif

      /* Number of blocks allocated (-1 if not available)  */
#if HAVE_STRUCT_STAT_ST_BLOCKS
      if (sb.st_blocks <= INT32_MAX)
	ARRAY_ELEM_I4(values, 12) = sb.st_blocks;
#endif
    }
	       ...
	       gfc_charlen_type name_len, int is_lstat __attribute__ ((unused)))
{
  ...
  if (val == 0)
    {
      /* Device ID  */
      ARRAY_ELEM_I8(values, 0) = sb.st_dev;

      /* Inode number  */
      ARRAY_ELEM_I8(values, 1) = sb.st_ino;

      /* File mode  */
      ARRAY_ELEM_I8(values, 2) = sb.st_mode;

      /* Number of (hard) links  */
      ARRAY_ELEM_I8(values, 3) = sb.st_nlink;

      /* Owner's uid  */
      ARRAY_ELEM_I8(values, 4) = sb.st_uid;

      /* Owner's gid  */
      ARRAY_ELEM_I8(values, 5) = sb.st_gid;

      /* ID of device containing directory entry for file (0 if not available) */
#if HAVE_STRUCT_STAT_ST_RDEV
      ARRAY_ELEM_I8(values, 6) = sb.st_rdev;
#else
      ARRAY_ELEM_I8(values, 6) = 0;
#endif

      /* File size (bytes)  */
      ARRAY_ELEM_I8(values, 7) = sb.st_size;

      /* Last access time  */
      ARRAY_ELEM_I8(values, 8) = sb.st_atime;

      /* Last modification time  */
      ARRAY_ELEM_I8(values, 9) = sb.st_mtime;

      /* Last file status change time  */
      ARRAY_ELEM_I8(values, 10) = sb.st_ctime;

      /* Preferred I/O block size (-1 if not available)  */
#if HAVE_STRUCT_STAT_ST_BLKSIZE
      ARRAY_ELEM_I8(values, 11) = sb.st_blksize;
#else
      ARRAY_ELEM_I8(values, 11) = -1;
#endif

      /* Number of blocks allocated (-1 if not available)  */
#if HAVE_STRUCT_STAT_ST_BLOCKS
      ARRAY_ELEM_I8(values, 12) = sb.st_blocks;
#else
      ARRAY_ELEM_I8(values, 12) = -1;
#endif
    }
