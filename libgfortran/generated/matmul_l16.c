	...
	gfc_array_l1 * const restrict a, gfc_array_l1 * const restrict b)
{
  ...
  if (GFC_DESCRIPTOR_RANK (retarray) == 1)
    {
      rxstride = GFC_DESCRIPTOR_SPACING(retarray,0);
      rystride = rxstride;
    }
  else
    {
      rxstride = GFC_DESCRIPTOR_SPACING(retarray,0);
      rystride = GFC_DESCRIPTOR_SPACING(retarray,1);
    }
      ...
      for (x = 0; x < xcount; x++)
        {
          ...
          *dest = 0;
          ...
          dest = (GFC_LOGICAL_16*) (((char*)dest) + rxstride);
          abase += xstride;
        }
      abase -= xstride * xcount;
      bbase += ystride;
      dest = (GFC_LOGICAL_16*) (((char*)dest) + rystride - (rxstride * xcount));
    }
}
