  ...
  while (sptr)
    {
      ...
      for (n = 0; n < ncopies; n++)
        {
	  *dest = *sptr;
          dest = (GFC_REAL_10*) (((char*) dest) + rdelta);
        }
      /* Advance to the next element.  */
      sptr = (GFC_REAL_10*) (((char*) sptr) + sspacing0);
      ...
      while (count[n] == extent[n])
        {
          ...
          sptr = (GFC_REAL_10*) (((char*) sptr) - sspacing[n] * extent[n]);
          rptr = (GFC_REAL_10*) (((char*) rptr) - rspacing[n] * extent[n]);
          n++;
          if (n >= srank)
            {
            ...
            {
              count[n]++;
              sptr = (GFC_REAL_10*) (((char*) sptr) + sspacing[n]);
			...
			const index_type along, const index_type ncopies)
{
  ...
  if (ret->base_addr == NULL)
    {
      ...
      ret->offset = 0;
      GFC_DESCRIPTOR_DIMENSION_SET(ret, 0, 0, ncopies - 1, sizeof (GFC_REAL_10));
    }
  else
    {
  ...
  for (index_type n = 0; n < ncopies; n++)
    {
      *dest = *source;
      dest = (GFC_REAL_10*) (((char*) dest) + spacing);
    }
}
