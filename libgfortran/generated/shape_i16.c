	...
	const array_t * const restrict array)
{
  ...
  if (ret->base_addr == NULL)
    {
      GFC_DESCRIPTOR_DIMENSION_SET(ret, 0, 0, rank - 1, sizeof (GFC_INTEGER_16));
      ret->offset = 0;
      ret->base_addr = xmalloc (rank);
    }
  ...
  for (index_type n = 0; n < rank; n++)
    {
      ...
      *((GFC_INTEGER_16*) (((char*) ret->base_addr) + n * spacing)) = extent > 0 ? extent : 0 ;
    }
}
