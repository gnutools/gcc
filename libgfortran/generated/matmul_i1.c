  ...
  if (GFC_DESCRIPTOR_RANK (a) == 1)
    {
      ...
      ayspacing = sizeof (GFC_UINTEGER_1);
...
#define max(a,b) ((a) >= (b) ? (a) : (b))
  ...
  if (try_blas
      && rxspacing == sizeof (GFC_UINTEGER_1)
      && (axspacing == sizeof (GFC_UINTEGER_1) || ayspacing == sizeof (GFC_UINTEGER_1))
      && (bxspacing == sizeof (GFC_UINTEGER_1) || byspacing == sizeof (GFC_UINTEGER_1))
      && (((float) xcount) * ((float) ycount) * ((float) count)
          > POW3(blas_limit)))
    {
      const int m = xcount, n = ycount, k = count, ldc = ryspacing;
      const GFC_UINTEGER_1 one = 1, zero = 0;
      const int lda = (axspacing == sizeof (GFC_UINTEGER_1)) ? ayspacing : axspacing,
		ldb = (bxspacing == sizeof (GFC_UINTEGER_1)) ? byspacing : bxspacing;
      ...
      if (lda > 0 && ldb > 0 && ldc > 0 && m > 1 && n > 1 && k > 1)
	{
	  ...
	  if (try_blas & 2)
	    ...
	    transa = axspacing == sizeof (GFC_UINTEGER_1) ? "N" : "T";

	  if (try_blas & 4)
	    transb = "C";
	  else
	    transb = bxspacing == sizeof (GFC_UINTEGER_1) ? "N" : "T";
  ...
  if (rxspacing == sizeof (GFC_UINTEGER_1)
      && axspacing == sizeof (GFC_UINTEGER_1)
      && bxspacing == sizeof (GFC_UINTEGER_1)
      && GFC_DESCRIPTOR_RANK (b) != 1)
    {
	 ...
	 from netlib.org, translated to C, and modified for matmul.m4.  */
      ...
      c = (GFC_UINTEGER_1 *) (((char*)c) - c_offset);
      a_dim0 = axspacing;
      a_dim1 = ayspacing;
      a_offset = axspacing + a_dim1;
      a = (GFC_UINTEGER_1 *) (((char*)a) - a_offset);
      b_dim0 = bxspacing;
      b_dim1 = byspacing;
      b_offset = bxspacing + b_dim1;
      b = (GFC_UINTEGER_1 *) (((char*)b) - b_offset);

      /* Empty c first.  */
      for (j=1; j<=n; j++)
	for (i=1; i<=m; i++)
	  GFC_ARRAY_ELEM (GFC_UINTEGER_1, c, i * rxspacing + j * c_dim1) = (GFC_UINTEGER_1)0;
	...
	return;
      ...
      if (ayspacing == sizeof (GFC_UINTEGER_1))
        a_sz = ryspacing;
      else
        a_sz = a_dim1;
		  ...
		  for (l = ll; l <= i4; l += 2)
		      ...
		      for (i = ii; i <= i5; i += 2)
			{
			  t1[l - ll + 1 + ((i - ii + 1) << 8) - 257] =
					GFC_ARRAY_ELEM (GFC_UINTEGER_1, a, i * a_dim0 + l * a_dim1);
			  t1[l - ll + 2 + ((i - ii + 1) << 8) - 257] =
					GFC_ARRAY_ELEM (GFC_UINTEGER_1, a, i * a_dim0 + (l + 1) * a_dim1);
			  t1[l - ll + 1 + ((i - ii + 2) << 8) - 257] =
					GFC_ARRAY_ELEM (GFC_UINTEGER_1, a, (i + 1) * a_dim0 + l * a_dim1);
			  t1[l - ll + 2 + ((i - ii + 2) << 8) - 257] =
					GFC_ARRAY_ELEM (GFC_UINTEGER_1, a, (i + 1) * a_dim0 + (l + 1) * a_dim1);
			}
		      if (uisec < isec)
			{
			  t1[l - ll + 1 + (isec << 8) - 257] =
				    GFC_ARRAY_ELEM (GFC_UINTEGER_1, a, (ii + isec - 1) * a_dim0 + l * a_dim1);
			  t1[l - ll + 2 + (isec << 8) - 257] =
				    GFC_ARRAY_ELEM (GFC_UINTEGER_1, a, (ii + isec - 1) * a_dim0 + (l + 1) * a_dim1);
			}
		    }
		  if (ulsec < lsec)
		    {
		      ...
		      for (i = ii; i<= i4; ++i)
			{
			  t1[lsec + ((i - ii + 1) << 8) - 257] =
				    GFC_ARRAY_ELEM (GFC_UINTEGER_1, a, i * a_dim0 + (ll + lsec - 1) * a_dim1);
			}
		    }
		  ...
		  for (j = jj; j <= i4; j += 4)
		    {
		      ...
		      for (i = ii; i <= i5; i += 4)
			{
			  f11 = GFC_ARRAY_ELEM (GFC_UINTEGER_1, c, i * c_dim0 + j * c_dim1);
			  f21 = GFC_ARRAY_ELEM (GFC_UINTEGER_1, c, (i + 1) * c_dim0 + j * c_dim1);
			  f12 = GFC_ARRAY_ELEM (GFC_UINTEGER_1, c, i * c_dim0 + (j + 1) * c_dim1);
			  f22 = GFC_ARRAY_ELEM (GFC_UINTEGER_1, c, (i + 1) * c_dim0 + (j + 1) * c_dim1);
			  f13 = GFC_ARRAY_ELEM (GFC_UINTEGER_1, c, i * c_dim0 + (j + 2) * c_dim1);
			  f23 = GFC_ARRAY_ELEM (GFC_UINTEGER_1, c, (i + 1) * c_dim0 + (j + 2) * c_dim1);
			  f14 = GFC_ARRAY_ELEM (GFC_UINTEGER_1, c, i * c_dim0 + (j + 3) * c_dim1);
			  f24 = GFC_ARRAY_ELEM (GFC_UINTEGER_1, c, (i + 1) * c_dim0 + (j + 3) * c_dim1);
			  f31 = GFC_ARRAY_ELEM (GFC_UINTEGER_1, c, (i + 2) * c_dim0 + j * c_dim1);
			  f41 = GFC_ARRAY_ELEM (GFC_UINTEGER_1, c, (i + 3) * c_dim0 + j * c_dim1);
			  f32 = GFC_ARRAY_ELEM (GFC_UINTEGER_1, c, (i + 2) * c_dim0 + (j + 1) * c_dim1);
			  f42 = GFC_ARRAY_ELEM (GFC_UINTEGER_1, c, (i + 3) * c_dim0 + (j + 1) * c_dim1);
			  f33 = GFC_ARRAY_ELEM (GFC_UINTEGER_1, c, (i + 2) * c_dim0 + (j + 2) * c_dim1);
			  f43 = GFC_ARRAY_ELEM (GFC_UINTEGER_1, c, (i + 3) * c_dim0 + (j + 2) * c_dim1);
			  f34 = GFC_ARRAY_ELEM (GFC_UINTEGER_1, c, (i + 2) * c_dim0 + (j + 3) * c_dim1);
			  f44 = GFC_ARRAY_ELEM (GFC_UINTEGER_1, c, (i + 3) * c_dim0 + (j + 3) * c_dim1);
			  i6 = ll + lsec - 1;
			  for (l = ll; l <= i6; ++l)
			    {
			      f11 += t1[l - ll + 1 + ((i - ii + 1) << 8) - 257]
				      * GFC_ARRAY_ELEM (GFC_UINTEGER_1, b, l * b_dim0 + j * b_dim1);
			      f21 += t1[l - ll + 1 + ((i - ii + 2) << 8) - 257]
				      * GFC_ARRAY_ELEM (GFC_UINTEGER_1, b, l * b_dim0 + j * b_dim1);
			      f12 += t1[l - ll + 1 + ((i - ii + 1) << 8) - 257]
				      * GFC_ARRAY_ELEM (GFC_UINTEGER_1, b, l * b_dim0 + (j + 1) * b_dim1);
			      f22 += t1[l - ll + 1 + ((i - ii + 2) << 8) - 257]
				      * GFC_ARRAY_ELEM (GFC_UINTEGER_1, b, l * b_dim0 + (j + 1) * b_dim1);
			      f13 += t1[l - ll + 1 + ((i - ii + 1) << 8) - 257]
				      * GFC_ARRAY_ELEM (GFC_UINTEGER_1, b, l * b_dim0 + (j + 2) * b_dim1);
			      f23 += t1[l - ll + 1 + ((i - ii + 2) << 8) - 257]
				      * GFC_ARRAY_ELEM (GFC_UINTEGER_1, b, l * b_dim0 + (j + 2) * b_dim1);
			      f14 += t1[l - ll + 1 + ((i - ii + 1) << 8) - 257]
				      * GFC_ARRAY_ELEM (GFC_UINTEGER_1, b, l * b_dim0 + (j + 3) * b_dim1);
			  ...
			  GFC_ARRAY_ELEM (GFC_UINTEGER_1, c, i * c_dim0 + j * c_dim1) = f11;
			  GFC_ARRAY_ELEM (GFC_UINTEGER_1, c, (i + 1) * c_dim0 + j * c_dim1) = f21;
			  GFC_ARRAY_ELEM (GFC_UINTEGER_1, c, i * c_dim0 + (j + 1) * c_dim1) = f12;
			  GFC_ARRAY_ELEM (GFC_UINTEGER_1, c, (i + 1) * c_dim0 + (j + 1) * c_dim1) = f22;
			  GFC_ARRAY_ELEM (GFC_UINTEGER_1, c, i * c_dim0 + (j + 2)  * c_dim1) = f13;
			  GFC_ARRAY_ELEM (GFC_UINTEGER_1, c, (i + 1) * c_dim0 + (j + 2) * c_dim1) = f23;
			  GFC_ARRAY_ELEM (GFC_UINTEGER_1, c, i * c_dim0 + (j + 3)  * c_dim1) = f14;
			  GFC_ARRAY_ELEM (GFC_UINTEGER_1, c, (i + 1) * c_dim0 + (j + 3) * c_dim1) = f24;
			  GFC_ARRAY_ELEM (GFC_UINTEGER_1, c, (i + 2) * c_dim0 + j * c_dim1) = f31;
			  GFC_ARRAY_ELEM (GFC_UINTEGER_1, c, (i + 3) * c_dim0 + j * c_dim1) = f41;
			  GFC_ARRAY_ELEM (GFC_UINTEGER_1, c, (i + 2) * c_dim0 + (j + 1) * c_dim1) = f32;
			  GFC_ARRAY_ELEM (GFC_UINTEGER_1, c, (i + 3) * c_dim0 + (j + 1) * c_dim1) = f42;
			  GFC_ARRAY_ELEM (GFC_UINTEGER_1, c, (i + 2) * c_dim0 + (j + 2) * c_dim1) = f33;
			  GFC_ARRAY_ELEM (GFC_UINTEGER_1, c, (i + 3) * c_dim0 + (j + 2) * c_dim1) = f43;
			  GFC_ARRAY_ELEM (GFC_UINTEGER_1, c, (i + 2) * c_dim0 + (j + 3) * c_dim1) = f34;
			  GFC_ARRAY_ELEM (GFC_UINTEGER_1, c, (i + 3) * c_dim0 + (j + 3) * c_dim1) = f44;
			}
		      if (uisec < isec)
			{
			  i5 = ii + isec - 1;
			  for (i = ii + uisec; i <= i5; ++i)
			      ...
			      for (l = ll; l <= i6; ++l)
				{
				  f11 += t1[l - ll + 1 + ((i - ii + 1) << 8) -
					  257] * GFC_ARRAY_ELEM (GFC_UINTEGER_1, b, l * b_dim0 + j * b_dim1);
				  f12 += t1[l - ll + 1 + ((i - ii + 1) << 8) -
					  257] * GFC_ARRAY_ELEM (GFC_UINTEGER_1, b, l * b_dim0 + (j + 1) * b_dim1);
				  f13 += t1[l - ll + 1 + ((i - ii + 1) << 8) -
					  257] * GFC_ARRAY_ELEM (GFC_UINTEGER_1, b, l * b_dim0 + (j + 2) * b_dim1);
				  f14 += t1[l - ll + 1 + ((i - ii + 1) << 8) -
					  257] * GFC_ARRAY_ELEM (GFC_UINTEGER_1, b, l * b_dim0 + (j + 3) * b_dim1);
				}
			      GFC_ARRAY_ELEM (GFC_UINTEGER_1, c, i * c_dim0 + j * c_dim1) = f11;
			      GFC_ARRAY_ELEM (GFC_UINTEGER_1, c, i * c_dim0 + (j + 1) * c_dim1) = f12;
			      GFC_ARRAY_ELEM (GFC_UINTEGER_1, c, i * c_dim0 + (j + 2) * c_dim1) = f13;
  ...
  else if (rxspacing == sizeof (GFC_UINTEGER_1)
	   && ayspacing == sizeof (GFC_UINTEGER_1)
	   && bxspacing == sizeof (GFC_UINTEGER_1))
    {
      if (GFC_DESCRIPTOR_RANK (a) != 1)
	{
	  ...
	  for (y = 0; y < ycount; y++)
	    {
	      bbase_y = GFC_DESCRIPTOR_DIM_ELEM_ADDRESS (const GFC_UINTEGER_1 * restrict, b, 1, y);
	      dest_y = GFC_DESCRIPTOR_DIM_ELEM_ADDRESS (GFC_UINTEGER_1 * restrict, retarray, 1, y);
	      for (x = 0; x < xcount; x++)
		{
		  abase_x = GFC_DESCRIPTOR_DIM_ELEM_ADDRESS (const GFC_UINTEGER_1 * restrict, a, 0, x);
		  s = (GFC_UINTEGER_1) 0;
		  for (n = 0; n < count; n++)
	  ...
	  for (y = 0; y < ycount; y++)
	    {
	      ...
	      s = (GFC_UINTEGER_1) 0;
	      for (n = 0; n < count; n++)
		s += GFC_DESCRIPTOR_DIM_ELEM (const GFC_UINTEGER_1, a, 0, n) * bbase_y[n];
	      GFC_DESCRIPTOR_DIM_ELEM (GFC_UINTEGER_1, retarray, 0, y) = s;
	    }
	}
    }
      ...
      for (y = 0; y < ycount; y++)
	{
	  ...
	  s = (GFC_UINTEGER_1) 0;
	  for (n = 0; n < count; n++)
	    s += GFC_DESCRIPTOR_DIM_ELEM (const GFC_UINTEGER_1, a, 0, n)
		 * GFC_ARRAY_ELEM (const GFC_UINTEGER_1, bbase_y, n * bxspacing);
	  GFC_DESCRIPTOR_DIM_ELEM (GFC_UINTEGER_1, retarray, 0, y) = s;
	}
    }
  else if (axspacing < ayspacing)
    {
      for (y = 0; y < ycount; y++)
	for (x = 0; x < xcount; x++)
	  GFC_ARRAY_ELEM (GFC_UINTEGER_1, dest, x*rxspacing + y*ryspacing) = (GFC_UINTEGER_1)0;

      for (y = 0; y < ycount; y++)
	for (n = 0; n < count; n++)
	  for (x = 0; x < xcount; x++)
	    /* dest[x,y] += a[x,n] * b[n,y] */
	    GFC_ARRAY_ELEM (GFC_UINTEGER_1, dest, x*rxspacing + y*ryspacing)
		+= GFC_ARRAY_ELEM (GFC_UINTEGER_1, abase, x*axspacing + n*ayspacing)
		   * GFC_ARRAY_ELEM (GFC_UINTEGER_1, bbase, n*bxspacing + y*byspacing);
    }
  else
    {
      const GFC_UINTEGER_1 *restrict abase_x;
	  ...
	  for (x = 0; x < xcount; x++)
	    {
	      ...
	      s = (GFC_UINTEGER_1) 0;
	      for (n = 0; n < count; n++)
		s += GFC_ARRAY_ELEM (const GFC_UINTEGER_1, abase_x, n*ayspacing)
		     * GFC_ARRAY_ELEM (const GFC_UINTEGER_1, bbase_y, n*bxspacing);
	      GFC_ARRAY_ELEM (GFC_UINTEGER_1, dest_y, x*rxspacing) = s;
	    }
	}
    }
}
