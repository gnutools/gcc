  ...
  if (unlikely (x == 0))
    {
      ret->base_addr[0] = 1;
      for (i = 1; i <= n2-n1; i++)
        *((GFC_REAL_10*) (((char*) ret->base_addr) + i*spacing)) = 0;
      return;
    }

  last1 = MATHFUNC(jn) (n2, x);
  *((GFC_REAL_10*) (((char*) ret->base_addr) + (n2-n1)*spacing)) = last1;

  if (n1 == n2)
    return;

  last2 = MATHFUNC(jn) (n2 - 1, x);
  *((GFC_REAL_10*) (((char*) ret->base_addr) + (n2-n1-1)*spacing)) = last2;
  ...
  for (i = n2-n1-2; i >= 0; i--)
    {
      *((GFC_REAL_10*) (((char*) ret->base_addr) + i*spacing)) = x2rev * (i+1+n1) * last2 - last1;
      last1 = last2;
      last2 = *((GFC_REAL_10*) (((char*) ret->base_addr) + i*spacing));
    }
}
			 ...
			 GFC_REAL_10 x)
{
  ...
  if (unlikely (x == 0))
    {
      for (i = 0; i <= n2-n1; i++)
        ...
        *((GFC_REAL_10*) (((char*) ret->base_addr) + i*spacing)) = -GFC_REAL_10_INFINITY;
#else
        *((GFC_REAL_10*) (((char*) ret->base_addr) + i*spacing)) = -GFC_REAL_10_HUGE;
#endif
      return;
    }
  ...
  *((GFC_REAL_10*) (((char*) ret->base_addr) + 1*spacing)) = last2;
