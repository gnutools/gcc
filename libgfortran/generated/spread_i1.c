  ...
  while (sptr)
    {
      ...
      for (n = 0; n < ncopies; n++)
        {
	  *dest = *sptr;
          dest = (GFC_INTEGER_1*) (((char*) dest) + rdelta);
        }
      ...
      while (count[n] == extent[n])
        {
          ...
          sptr = (GFC_INTEGER_1*) (((char*) sptr) - sspacing[n] * extent[n]);
			...
			const index_type along, const index_type ncopies)
{
  ...
  if (ret->base_addr == NULL)
    {
      ...
      ret->offset = 0;
      GFC_DESCRIPTOR_DIMENSION_SET(ret, 0, 0, ncopies - 1, sizeof (GFC_INTEGER_1));
    }
  else
    {
  ...
  for (index_type n = 0; n < ncopies; n++)
    {
      *dest = *source;
      dest = (GFC_INTEGER_1*) (((char*) dest) + spacing);
    }
}
