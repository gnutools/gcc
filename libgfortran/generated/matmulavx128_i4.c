...
#define A_ARRAY_ELEM(i,j) \
    (ARRAY_ELEM_AT_OFFSET (abase, (i) * sizeof (GFC_INTEGER_4) + (j) * aystride_bytes))

#define B_ARRAY_ELEM(i,j) \
    (ARRAY_ELEM_AT_OFFSET (bbase, (i) * sizeof (GFC_INTEGER_4) + (j) * bystride_bytes))

#define C_ARRAY_ELEM(i,j) \
    (ARRAY_ELEM_AT_OFFSET (c, (i) * sizeof (GFC_INTEGER_4) + (j) * rystride_bytes))

      /* Empty result first.  */
      for (j=0; j<n; j++)
	for (i=0; i<m; i++)
	  C_ARRAY_ELEM (i, j) = (GFC_INTEGER_4)0;
...
#undef C_ARRAY_ELEM
  ...
  else if (axstride_bytes < aystride_bytes)
    {
      for (y = 0; y < ycount; y++)
	for (x = 0; x < xcount; x++)
	  GFC_DESCRIPTOR2_ELEM (retarray, x, y) = (GFC_INTEGER_4)0;
