  ...
  while (rptr)
    {
	  ...
	  /* From vector.  */
	  *rptr = *vptr;
	  vptr = (GFC_REAL_17*) (((char*) vptr) + vspacing0);
        }
      else
        {
	  /* From field.  */
  ...
  while (rptr)
    {
	  ...
	  *rptr = *fptr;
      ...
      /* Advance to the next element.  */
      rptr = (GFC_REAL_17*) (((char*) rptr) + rspacing0);
      fptr = (GFC_REAL_17*) (((char*) rptr) + fspacing0);
      mptr += mspacing0;
      count[0]++;
      n = 0;
      while (count[n] == extent[n])
        {
          ...
          /* We could precalculate these products, but this is a less
             frequently used path so probably not worth it.  */
          rptr = (GFC_REAL_17*) (((char*) rptr) - rspacing[n] * extent[n]);
          fptr = (GFC_REAL_17*) (((char*) fptr) - fspacing[n] * extent[n]);
          mptr -= mspacing[n] * extent[n];
          n++;
          if (n >= dim)
            {
          ...
          else
            {
              count[n]++;
              rptr = (GFC_REAL_17*) (((char*) rptr) + rspacing[n]);
              fptr = (GFC_REAL_17*) (((char*) fptr) + fspacing[n]);
              mptr += mspacing[n];
            }
        }
    }
