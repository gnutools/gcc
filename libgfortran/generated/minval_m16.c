	...
	const index_type * const restrict pdim)
{
  ...
  if (retarray->base_addr == NULL)
      ...
      for (n = 0; n < rank; n++)
	{
      ...
      GFC_DESCRIPTOR_SIZE (retarray) = sizeof (GFC_UINTEGER_16);
      GFC_DESCRIPTOR_SPAN (retarray) = sizeof (GFC_UINTEGER_16);
  ...
  while (continue_loop)
	...
	if (len <= 0)
	  *dest = GFC_UINTEGER_16_HUGE;
      ...
      dest = (GFC_UINTEGER_16*) (((char*)dest) + dspacing[0]);
      n = 0;
      while (count[n] == extent[n])
	{
	  ...
	  dest = (GFC_UINTEGER_16*) (((char*)dest) - dspacing[n] * extent[n]);
	  n++;
	  if (n >= rank)
	    {
	  ...
	  else
	    {
	      ...
	      dest = (GFC_UINTEGER_16*) (((char*)dest) + dspacing[n]);
	    }
	}
    }
}
