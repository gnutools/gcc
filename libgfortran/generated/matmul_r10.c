...
#define max(a,b) ((a) >= (b) ? (a) : (b))
  ...
  if (rxspacing == sizeof (GFC_REAL_10)
      ...
      if (GFC_DESCRIPTOR_RANK (a) != 1)
	{
	  ...
	  for (y = 0; y < ycount; y++)
	    {
	      bbase_y = GFC_DESCRIPTOR_DIM_ELEM (const GFC_REAL_10 * restrict, b, 1, y);
	      dest_y = GFC_DESCRIPTOR_DIM_ELEM (GFC_REAL_10 * restrict, retarray, 1, y);
	      for (x = 0; x < xcount; x++)
		{
		  abase_x = GFC_DESCRIPTOR_DIM_ELEM (const GFC_REAL_10 * restrict, a, 0, x);
		  s = (GFC_REAL_10) 0;
		  for (n = 0; n < count; n++)
