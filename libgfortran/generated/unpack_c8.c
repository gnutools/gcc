		 ...
		 const gfc_array_l1 *mask, const GFC_COMPLEX_8 *fptr)
{
  ...
  if (ret->base_addr == NULL)
      ...
      for (n = 0; n < dim; n++)
	{
      ...
      if (rspacing[0] == 0)
	rspacing[0] = sizeof (GFC_COMPLEX_8);
    }
  ...
  if (vspacing0 == 0)
    vspacing0 = sizeof (GFC_COMPLEX_8);
  ...
  while (rptr)
    {
	  ...
	  /* From vector.  */
	  *rptr = *vptr;
	  vptr = (GFC_COMPLEX_8*) (((char*) vptr) + vspacing0);
        }
      else
        {
	  /* From field.  */
		 ...
		 const gfc_array_l1 *mask, const gfc_array_c8 *field)
{
  ...
  if (fspacing[0] == 0)
    fspacing[0] = sizeof (GFC_COMPLEX_8);
  if (mspacing[0] == 0)
    mspacing[0] = 1;
  ...
  while (rptr)
    {
	  ...
	  *rptr = *fptr;
      ...
      /* Advance to the next element.  */
      rptr = (GFC_COMPLEX_8*) (((char*) rptr) + rspacing0);
      fptr = (GFC_COMPLEX_8*) (((char*) rptr) + fspacing0);
      mptr += mspacing0;
      count[0]++;
      n = 0;
      while (count[n] == extent[n])
        {
          ...
          /* We could precalculate these products, but this is a less
             frequently used path so probably not worth it.  */
          rptr = (GFC_COMPLEX_8*) (((char*) rptr) - rspacing[n] * extent[n]);
          fptr = (GFC_COMPLEX_8*) (((char*) fptr) - fspacing[n] * extent[n]);
          mptr -= mspacing[n] * extent[n];
          n++;
          if (n >= dim)
            {
          ...
          else
            {
              count[n]++;
              rptr = (GFC_COMPLEX_8*) (((char*) rptr) + rspacing[n]);
              fptr = (GFC_COMPLEX_8*) (((char*) fptr) + fspacing[n]);
              mptr += mspacing[n];
            }
        }
    }
