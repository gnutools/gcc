...
#define max(a,b) ((a) >= (b) ? (a) : (b))
  ...
  if (rxspacing == sizeof (GFC_INTEGER_2)
      ...
      if (GFC_DESCRIPTOR_RANK (a) != 1)
	{
	  ...
	  for (y = 0; y < ycount; y++)
	    {
	      bbase_y = GFC_DESCRIPTOR_DIM_ELEM (const GFC_INTEGER_2 * restrict, b, 1, y);
	      dest_y = GFC_DESCRIPTOR_DIM_ELEM (GFC_INTEGER_2 * restrict, retarray, 1, y);
	      for (x = 0; x < xcount; x++)
		{
		  abase_x = GFC_DESCRIPTOR_DIM_ELEM (const GFC_INTEGER_2 * restrict, a, 0, x);
		  s = (GFC_INTEGER_2) 0;
		  for (n = 0; n < count; n++)
