...
rs6000_debug_reg_global (void)
{
  ...
  if (rs6000_cpu_index >= 0)
	...
	= processor_target_table[rs6000_cpu_index].target_enable;
      HOST_WIDE_INT cpu_isa
	= processor_target_table[rs6000_cpu_index].cpu_isa;

      sprintf (flags_buffer, "-mcpu=%s flags", name);
      rs6000_print_isa_options (stderr, 0, flags_buffer, flags, cpu_isa);
    }
  else
  ...
  rs6000_print_isa_options (stderr, 0, "rs6000_isa_flags",
			    rs6000_isa_flags, rs6000_cpu_isa_flags);
  ...
  if (cpu_index >= 0)
    {
      ...
      rs6000_isa_flags |= (processor_target_table[cpu_index].target_enable
			   & set_masks);
      rs6000_cpu_isa_flags = processor_target_table[cpu_index].cpu_isa;
    }
  else
    {
...
rs6000_reverse_condition (machine_mode mode,
			  enum rtx_code code,
			  enum cond_reverse_fp do_reverse)
{
  /* Reversal of FP compares takes care -- an ordered compare
     ...
     when cond_reverse_fp::reverse_ok is passed.  */
  if (mode == CCFPmode)
    {
	 ...
	 passed.  */
      if (!flag_finite_math_only)
	return (do_reverse == cond_reverse_fp::no_reverse
		? UNKNOWN
...
static struct rs6000_opt_mask const rs6000_cpu_isa_masks[] =
{
  { "future",			CPU_ISA_MASK_FUTURE,		false, true  },
};
...
rs6000_pragma_target_parse (tree args, tree pop_target)
{
  ...
  HOST_WIDE_INT prev_cpu_isa, cur_cpu_isa, diff_cpu_isa;
  ...
  if (rs6000_target_modify_macros_ptr)
    {
      ...
      prev_cpu_isa = prev_opt->x_rs6000_cpu_isa_flags;

      cur_opt      = TREE_TARGET_OPTION (cur_tree);
      cur_flags    = cur_opt->x_rs6000_isa_flags;
      cur_cpu_isa  = cur_opt->x_rs6000_cpu_isa_flags;

      diff_flags   = (prev_flags ^ cur_flags);
      diff_cpu_isa = (prev_cpu_isa ^ cur_cpu_isa);

      if (diff_flags != 0 || diff_cpu_isa != 0)
	{
	  /* Delete old macros.  */
	  rs6000_target_modify_macros_ptr (false,
					   prev_flags & diff_flags,
					   prev_cpu_isa & diff_cpu_isa);

	  /* Define new macros.  */
	  rs6000_target_modify_macros_ptr (true,
					   cur_flags & diff_flags,
					   cur_cpu_isa & diff_cpu_isa);
	}
    }
...
rs6000_print_options_internal (FILE *file,
			       ...
			       size_t num_elements,
			       HOST_WIDE_INT cpu_isa,
			       const char *cpu_isa_prefix,
			       const struct rs6000_opt_mask *cpu_isa_opts,
			       size_t num_cpu_isa_elements)
{
  size_t i;
  ...
  start_column += fprintf (stderr, DEBUG_FMT_WX, string, flags, cpu_isa);
  ...
  for (i = 0; i < num_cpu_isa_elements; i++)
    {
      const char *name = cpu_isa_opts[i].name;
      HOST_WIDE_INT mask = cpu_isa_opts[i].mask;
      size_t len = comma_len + prefix_len + strlen (name);
      ...
      if ((cpu_isa & mask) != 0)
	{
	  ...
	  fprintf (file, "%s%s%s", comma, cpu_isa_prefix, name);
	  comma = ", ";
	  comma_len = strlen (", ");
	}
    }
...
rs6000_print_isa_options (FILE *file, int indent, const char *string,
			  HOST_WIDE_INT flags, HOST_WIDE_INT cpu_isa)
{
  rs6000_print_options_internal (file, indent, string, flags, "-m",
				 &rs6000_opt_masks[0],
				 ARRAY_SIZE (rs6000_opt_masks),
				 cpu_isa, "cpu-isa=",
				 &rs6000_cpu_isa_masks[0],
				 ARRAY_SIZE (rs6000_cpu_isa_masks));
}
