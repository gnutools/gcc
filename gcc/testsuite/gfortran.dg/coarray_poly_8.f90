...
! { dg-final { scan-tree-dump-times {foo \(&class.[0-9]+, (?:NON_LVALUE_EXPR <)y._data.token>?, \(integer\(kind=[48]\)\) class.[0-9]+._data.data - \(integer\(kind=[48]\)\) y._data.data\);} 1 "original" } }
